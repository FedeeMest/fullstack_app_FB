...
async function findLegajo(req: Request, res: Response) {
    ...
    try {
        const alumno = await repository.findByLegajo(legajo);
        if (!alumno) {
            return res.status(404).json({ Error: 'Alumno no encontrado.' });
        }
        res.status(200).json(alumno);
...
async function add(req: Request, res: Response) {
    ...
    try {
        ...
        res.status(201).json({ message: 'Alumno creado con éxito', data: alumno });
    } catch (error) {
...
async function update(req: Request, res: Response) {
    const id = req.params.id;
    try {
        const alumno = await repository.update(id, req.body.inputS);
        if (!alumno) {
            return res.status(404).json({ Error: 'Alumno no encontrado.' });
        }
