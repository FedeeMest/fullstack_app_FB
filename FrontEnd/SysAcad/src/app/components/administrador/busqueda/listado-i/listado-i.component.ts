  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe((params) => {
      ...
      if (!isNaN(id) && id > 0) {
        ...
        this.getInscripciones(); // Obtener las inscripciones del alumno
      } else {
        console.error('No se recibió un ID válido en los queryParams');
        this.goBack(); // Volver a la página anterior si el ID no es válido
      }
    });
  }
    ...
    if (this.alumnoId) {
      this.alumnoService.getInscripcionesByAlumnoId(this.alumnoId).subscribe({
        next: (inscripciones: Inscripcion[]) => {
          ...
          this.loadMateriaNames(this.inscripciones); // Cargar los nombres de las materias
        },
        error: (err) => {
